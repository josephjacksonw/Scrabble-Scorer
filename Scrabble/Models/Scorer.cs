...
namespace Scorer.Models
{
  public class Game
  {
    public int tallyScore(string word)
    {
      ...
      // we need to analyze each letter in the inputted word
      //unless we can go down the line in a string, we're probably gonna have to convert the input to either a list or array
      // then just for loop it
      return score;
    }
  }
}